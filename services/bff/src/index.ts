...
import { filterObject, transformContentListForProfile } from './utils';
...
app.post('/content/:contentId', allowIfAnyOf('contentEditor'), async (req: Request, res: Response) => {
  const { status, data } = await contentApi.post('content/' + req.params.contentId);
  res.status(status).json(data);
});
...
app.get('/profiles/:id', async (req: Request, res: Response) => {
  ...
  const { files, errors: fileErrors } = await cloudflareApi.post('files', [
    profile?.heroFileId,
    profile?.logoFileId,
...
app.get('/', async (req: Request, res: Response) => {
  ...
  const { data: contentData } = await contentApi.get('/');
  const { data: cloudflareData } = await cloudflareApi.get('/');
  const { data: identityData } = await identityApi.get('/');

  res
    .status(200)
    .json({
      profileApi: profileData,
      contentApi: contentData,
      cloudflareApi: cloudflareData,
